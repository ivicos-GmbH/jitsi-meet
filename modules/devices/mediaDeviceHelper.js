...
import { getAudioOutputDeviceId, notifyCameraError, notifyMicError } from '../../react/features/base/devices';
import { getUserSelectedCameraDeviceId, getUserSelectedMicDeviceId, getUserSelectedOutputDeviceId } from '../../react/features/base/settings';
...
function getNewAudioOutputDevice(newDevices) {
    ...
    const availableAudioOutputDevices = newDevices.filter((d) => d.kind === 'audiooutput');

    // Switch to 'default' audio output device if we don't have the selected one
    // available anymore.
    if (selectedAudioOutputDeviceId !== 'default' && !availableAudioOutputDevices.find((d) => d.deviceId === selectedAudioOutputDeviceId)) {
        return 'default';
    }
...
function getNewAudioInputDevice(newDevices, localAudio) {
    const availableAudioInputDevices = newDevices.filter((d) => d.kind === 'audioinput');
    const selectedAudioInputDeviceId = getUserSelectedMicDeviceId(APP.store.getState());
    const selectedAudioInputDevice = availableAudioInputDevices.find((d) => d.deviceId === selectedAudioInputDeviceId);
    ...
    if (!localAudio || localAudio.disposed || localAudio.isEnded()) {
        ...
        } else if (availableAudioInputDevices.length && availableAudioInputDevices[0].label !== '') {
            return availableAudioInputDevices[0].deviceId;
        }
    } else if (selectedAudioInputDevice && selectedAudioInputDeviceId !== localAudio.getDeviceId()) {
...
function getNewVideoInputDevice(newDevices, localVideo) {
    const availableVideoInputDevices = newDevices.filter((d) => d.kind === 'videoinput');
    const selectedVideoInputDeviceId = getUserSelectedCameraDeviceId(APP.store.getState());
    const selectedVideoInputDevice = availableVideoInputDevices.find((d) => d.deviceId === selectedVideoInputDeviceId);
    ...
    if (!localVideo || localVideo.disposed || localVideo.isEnded()) {
        ...
        } else if (availableVideoInputDevices.length && availableVideoInputDevices[0].label !== '') {
            return availableVideoInputDevices[0].deviceId;
        }
    } else if (selectedVideoInputDevice && selectedVideoInputDeviceId !== localVideo.getDeviceId()) {
        // And here we handle case when we already have some device working,
        // but we plug-in a "preferred" (previously selected in settings, stored
        // in local storage) device.
        return selectedVideoInputDeviceId;
...
export default {
     ...
     *  }}
     */
    getNewMediaDevicesAfterDeviceListChanged(newDevices, isSharingScreen, localVideo, localAudio) { // eslint-disable-line max-params
        return {
     ...
     */
    createLocalTracksAfterDeviceListChanged(createLocalTracks, cameraDeviceId, micDeviceId) {
        let audioTrackError;
        let videoTrackError;
        ...
        if (audioRequested && videoRequested) {
            ...
            return (
                createLocalTracks({
                    ...
                    .catch(() => Promise.all([createAudioTrack(false).then(([stream]) => stream), createVideoTrack(false).then(([stream]) => stream)]))
                    .then((tracks) => {
                        if (audioTrackError) {
