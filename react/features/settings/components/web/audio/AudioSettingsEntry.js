...
export default function AudioSettingsEntry({ children, hasError, isSelected }: Props) {
    ...
    return (
        <div className={className}>
            {isSelected && <Icon className="audio-preview-icon audio-preview-icon--check" color="#1C2025" size={14} src={IconCheck} />}
            <span className="audio-preview-entry-text">{children}</span>
            {hasError && <Icon className="audio-preview-icon audio-preview-icon--exclamation" size={16} src={IconExclamationSolid} />}
        </div>
    );
}
