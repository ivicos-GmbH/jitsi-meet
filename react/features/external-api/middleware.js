...
import { CONFERENCE_FAILED, CONFERENCE_JOINED, KICKED_OUT } from '../base/conference';
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    ...
    switch (action.type) {
        case CONFERENCE_FAILED: {
            if (action.conference && action.error.name === JitsiConferenceErrors.PASSWORD_REQUIRED) {
        ...
        case CONFERENCE_JOINED: {
            ...
            APP.API.notifyConferenceJoined(room, id, {
                displayName: name,
                formattedDisplayName: appendSuffix(name, interfaceConfig.DEFAULT_LOCAL_DISPLAY_NAME),
        ...
        case PARTICIPANT_JOINED: {
            ...
            if (!local) {
                APP.API.notifyUserJoined(id, {
                    displayName: name,
                    formattedDisplayName: appendSuffix(name || interfaceConfig.DEFAULT_REMOTE_DISPLAY_NAME)
                });
            }
