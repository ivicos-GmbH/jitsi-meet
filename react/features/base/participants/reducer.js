...
function _participant(state: Object = {}, action) {
    switch (action.type) {
        ...
        case PARTICIPANT_ID_CHANGED: {
            ...
            if (state.id === action.oldValue && state.conference === conference && (conference || state.local)) {
                return {
                    ...state,
                    id: action.newValue
                };
...
function _participantJoined({ participant }) {
    const { avatarURL, botType, connectionStatus, dominantSpeaker, email, isFakeParticipant, isJigasi, loadableAvatarUrl, local, name, pinned, presence, role } = participant;
