import { API_URL_BROADCAST_STREAMS, API_URL_LIVE_BROADCASTS, DISCOVERY_DOCS, GOOGLE_SCOPE_CALENDAR, GOOGLE_SCOPE_YOUTUBE } from './constants';
...
const googleApi = {
    ...
    getCurrentUserProfile() {
        return this.get()
            ...
            .then((isSignedIn) => {
                ...
                return this._getGoogleApiClient().auth2.getAuthInstance().currentUser.get().getBasicProfile();
            });
    },
        ...
        return this.get().then(
            (api) =>
                new Promise((resolve, reject) => {
                    const scope = `${enableYoutube ? GOOGLE_SCOPE_YOUTUBE : ''} ${enableCalendar ? GOOGLE_SCOPE_CALENDAR : ''}`.trim();
    ...
    requestAvailableYouTubeBroadcasts() {
        return this.get().then((api) => api.client.request(API_URL_LIVE_BROADCASTS));
    },
    ...
    requestLiveStreamsForYouTubeBroadcast(boundStreamID) {
        return this.get().then((api) => api.client.request(`${API_URL_BROADCAST_STREAMS}${boundStreamID}`));
    },
    ...
    showAccountSelection() {
        return this.get().then((api) => api.auth2.getAuthInstance().signIn());
    },
     ...
     */
    signOut() {
        return this.get().then((api) => api.auth2 && api.auth2.getAuthInstance && api.auth2.getAuthInstance() && api.auth2.getAuthInstance().signOut());
    },
    ...
    _getConferenceDataVideoUri(conferenceData = {}) {
        try {
            ...
            if (conferenceData.parameters.addOnParameters.parameters.conferenceSolutionType === 'jitsi') {
        ...
        return this.get()
            ...
            .then((calendarList) => {
                ...
                return Promise.all(promises)
                    ...
                    .then((entries) => entries.map((e) => this._convertCalendarEntry(e)));
            });
    },
            ...
            .then((isSignedIn) => {
                ...
                return this._getGoogleApiClient()
                    ...
                    .then((event) => {
                        ...
                        return this._getGoogleApiClient().client.calendar.events.patch({
                            calendarId: calendarId,
                            eventId: id,
