...
import { openWebApp, openDesktopApp } from '../actions';
...
class DeepLinkingDesktopPage<P: Props> extends Component<P> {
    ...
    render() {
        ...
        const rightColumnStyle = SHOW_DEEP_LINKING_IMAGE ? null : { width: '100%' };
                    ...
                    <div className="content">
                        ...
                        <div className="rightColumn" style={rightColumnStyle}>
                            <div className="rightColumnContent">
                                <h1 className="title">{t(`${_TNS}.title`, { app: NATIVE_APP_NAME })}</h1>
                                <p className="description">{t(`${_TNS}.${isSupportedBrowser() ? 'description' : 'descriptionWithoutWeb'}`, { app: NATIVE_APP_NAME })}</p>
                                <div className="buttons">
                                    <ButtonGroup>
                                        <Button appearance="default" onClick={this._onTryAgain}>
                                            {t(`${_TNS}.tryAgainButton`)}
                                        </Button>
                                        {isSupportedBrowser() && <Button onClick={this._onLaunchWeb}>{t(`${_TNS}.launchWebButton`)}</Button>}
                                    </ButtonGroup>
                                </div>
                            </div>
                        </div>
