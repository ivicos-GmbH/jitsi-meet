...
import { getLocalVideoTrack, replaceLocalTrack, TRACK_ADDED, TRACK_NO_DATA_FROM_SOURCE } from '../base/tracks';

import { PREJOIN_START_CONFERENCE } from './actionTypes';
import { setDeviceStatusOk, setDeviceStatusWarning, setPrejoinPageVisibility } from './actions';
...
MiddlewareRegistry.register((store) => (next) => async (action) => {
    switch (action.type) {
        case PREJOIN_START_CONFERENCE: {
            ...
            const jitsiTracks = getState()
                ['features/base/tracks'].map((t) => t.jitsiTrack)
        ...
        case TRACK_NO_DATA_FROM_SOURCE: {
            ...
            if (isPrejoinPageVisible(state)) {
                ...
                if (track.isReceivingData()) {
                    if (deviceStatusType === 'warning' && deviceStatusText === 'prejoin.audioDeviceProblem') {
