...
import { PARTICIPANT_JOINED, PARTICIPANT_LEFT, PARTICIPANT_ROLE, PARTICIPANT_UPDATED, getParticipantById, getParticipantDisplayName } from '../base/participants';
import { MiddlewareRegistry, StateListenerRegistry } from '../base/redux';

import { clearNotifications, showNotification, showParticipantJoinedNotification } from './actions';
    ...
    switch (action.type) {
        case PARTICIPANT_JOINED: {
            ...
            if (!p.local && !joinLeaveNotificationsDisabled()) {
                dispatch(showParticipantJoinedNotification(getParticipantDisplayName(getState, p.id)));
            }

            if (typeof interfaceConfig === 'object' && !interfaceConfig.DISABLE_FOCUS_INDICATOR && p.role === PARTICIPANT_ROLE.MODERATOR) {
        ...
        case PARTICIPANT_LEFT: {
            if (!joinLeaveNotificationsDisabled()) {
                ...
                if (typeof interfaceConfig === 'object' && participant && !participant.local) {
                    store.dispatch(
                        showNotification(
                            {
